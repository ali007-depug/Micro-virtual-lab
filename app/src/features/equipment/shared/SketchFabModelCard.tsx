...
export function SketchfabModelCard({
  name, icon, description, modelUrl, modelScale, hotspots = [], interactiveKeys = [],
  accent = "colony", attribution,
}: Props) {
  const [interactionState, setInteractionState] = useState<Record<string, boolean>>({})
  const Icon = getIcon(icon)
  const toggle = (key: string) => setInteractionState((prev) => ({ ...prev, [key]: !prev[key] }))
  ...
  return (
    <Card className="overflow-hidden border-ink/10">
      <CardHeader className="flex flex-row items-center gap-3">
        <div className={`rounded-full p-2 ${accentBg[accent]}`}><Icon className="h-5 w-5" /></div>
      ...
      <CardContent className="space-y-3">
          ...
          <ModelErrorBoundary
            fallback={
              <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center">
                ...
                <p className="text-sm text-ink/60">3D model couldn't be loaded.</p>
              </div>
            }
          >
            <EquipmentCanvas>
        ...
        <p className="flex items-center justify-center gap-2 text-xs text-ink/50 font-mono">
          <MousePointerClick className="h-3.5 w-3.5" /> Drag to rotate · scroll to zoom
        </p>

        {attribution?.license === "CC-BY" && (
          <p className="text-center text-[11px] text-ink/40">
            Model by{" "}
            <a href={attribution.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline hover:text-culture">
              {attribution.author}
            </a>{" "}
            — CC-BY
          </p>
        ...
        <div className="rounded-lg bg-ink/5 p-4 text-sm text-ink/80 leading-relaxed">{description}</div>
      </CardContent>
    </Card>
  )
}